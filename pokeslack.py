...
transform_from_wgs_to_gcj(Location(Fort.Latitude, Fort.Longitude))
    ...
    for poke in visible:
        ...
        if distance < max_distance:
            ...
            alert_text = ':' + pokename.lower() + ':  ' + 'I\'m just <https://pokevision.com/#/@' + str(poke.Latitude) + ',' + str(poke.Longitude) + \
                         '|' + "{0:.2f}".format(distance) + \
                         ' m> ' + direction + ' until ' + disappear_time + \
                         ' (' + disappear_minutes + ':' + disappear_seconds + ')!' + '  #pokemongobot'

            if pokemon_icons_prefix != ':pokeball:':
                user_icon = ':' + pokename.lower() + ':'
            else:
                user_icon = ':pokeball:'
